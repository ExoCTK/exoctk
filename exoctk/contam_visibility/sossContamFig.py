...
def contam_test(cube, targetName='noName', paRange=[0, 360], badPA=[], tmpDir="",
    ...
    trace12 = cube[0, :, :]  # order 1 and 2
    cube = cube[1:, :, :]    # all the angles
...
# i think this is what's making the contam plot look weird
    for row in np.arange(rows):
        i = np.argmax(trace12[row, :])
        tr = trace12[row, i-20:i+41]
        w = tr/np.sum(tr**2)
        ww = np.tile(w, nPA).reshape([nPA, tr.size])
        contamO1[row, :] = np.sum(cube[:, row, i-20:i+41]*ww, axis=1)
        #contamO1[row, :] = np.sum(cube[:, row, :]*ww, axis=1)
