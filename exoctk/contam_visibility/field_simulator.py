...
from bokeh.models import Range1d, LinearColorMapper, Label, ColorBar
...
def calc_v3pa(V3PA, stars, aper, pixel_pos):
    """
    ...
    attitude = rotations.attitude_matrix(v2targ, v3targ, stars['ra'][0], stars['dec'][0], APA)

    # Get science coordinates of target
    sci_targx, sci_targy = stars['xsci'][0], stars['ysci'][0]

    # Iterate over stars in the FOV and add their traces to the data cube
    for idx, star in enumerate(stars):

        # Get the TEL coordinates of each star w attitude matrix
        V2, V3 = rotations.sky_to_tel(attitude, star['ra'], star['dec'])

        # Convert to arcsec and turn into a float
        stars[idx]['xtel'], stars[idx]['ytel'] = V2.to(u.arcsec).value, V3.to(u.arcsec).value

        # Save detector coords
        stars[idx]['xdet'], stars[idx]['ydet'] = aper.tel_to_det(stars[idx]['xtel'], stars[idx]['ytel'])
        stars[idx]['xsci'], stars[idx]['ysci'] = aper.det_to_sci(stars[idx]['xdet'], stars[idx]['ydet'])

        # See if star is in the FOV
        if (pixel_pos[0] < stars[idx]['ydet']) & (stars[idx]['ydet'] < pixel_pos[1]) & (pixel_pos[2] < stars[idx]['xdet']) & (stars[idx]['xdet'] < pixel_pos[3]):

            # Get the distance from the target star
            sci_dx = round(sci_targx - stars['xsci'][idx])
            sci_dy = round(sci_targy - stars['ysci'][idx])

            # Get the flux scaling
            fluxscale = 10.0**(-0.4 * (stars['j_m'][idx] - stars['j_m'][0]))

            # Get the trace and pad it
            trace = get_trace(aper.AperName, stars['Teff'][idx])
            ...
            (peakY,), (peakX,) = np.where(pad_trace == pad_trace.max())
            ...
            print('================================================')
            print('Star:', idx)
            print('Subarray:', subX, subY)
            print('Targ:', sci_targx, sci_targy)
            print('Dist:', sci_dx, sci_dy)
            print('Flux scale:', fluxscale)
            print('Peaks:', peakX, peakY)
            print('Targ loc:', xTarg, yTarg)
            print('Corners:', dimX0, dimX1, dimY0, dimY1)
            print('================================================')
...
def field_simulation(ra, dec, aperture, binComp='', n_jobs=-1, nPA=360):
    ...
    stars = Irsa.query_region(targetcrd, catalog='fp_psc', spatial='Cone', radius=inst['rad'] * u.arcmin)

    # Get distance to each FOV star and sort so that the target is the closest
    sindRA = (targetcrd.ra.value - stars['ra']) * np.cos(targetcrd.dec.value)
    cosdRA = targetcrd.dec.value - stars['dec']
    ...
    if binComp != '':
        ...
        star = {'ra': stars['ra'][0] + bb, 'dec': stars['dec'][0] + binComp[1] / 3600,
                'j_m': binComp[2], 'h_m': binComp[3], 'k_m': binComp[4]}
        stars.add_row(star)
    ...
    func = partial(calc_v3pa, stars=stars, aper=aper, pixel_pos=pixel_pos)
    cubes = zip(*pl.map(func, np.arange(0, nPA, 1)))
    pl.close()
    pl.join()

    # Sum cubes into a single cube
    cubes = np.asarray(list(cubes)).swapaxes(0, 1)
    simuCube = np.nansum(cubes, axis=0)
...
def plot_contamination(data, wlims, badPAs=[], minPA=0, maxPA=360, title=''):
    ...
    tooltips = [("(x,y)", "($x{int}, $y{int})"), ("Value", "@data")]
    tools = ['pan', 'box_zoom', 'crosshair', 'reset', 'hover']
    trplot = figure(tools=tools, tooltips=tooltips, width=500, height=500, title=title, x_range=Range1d(xlim0, xlim1), y_range=Range1d(ylim0, ylim1))
    trplot.image(source=source, image='data', x=xlim0, y=ylim0, dw=xlim1 - xlim0, dh=ylim1 - ylim0, color_mapper=color_mapper)
