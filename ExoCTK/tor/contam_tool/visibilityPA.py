...
def checkVisPA(ra,dec,targetName=None,save=False,tmpDir=""):
    ...
    plt.grid()
    if save:
        fName=tmpDir+'/visibilityPA-'+targetName
        pdf = fName+'.pdf'
        png = fName+'.png'
        plt.savefig(pdf)
        plt.savefig(png)
...
if __name__ == "__main__":
    ...
    argv = sys.argv
    targetName=None if len(argv)<4 else argv[3]
    save=False if len(argv)<4 else True
    tmpDir="" if len(argv)<5 else argv[4] 
    os.makedirs(tmpDir, exist_ok=True)

    checkVisPA(argv[1],argv[2],targetName=targetName,save=save,tmpDir=tmpDir)
