...
def leap_years(year1, year2):
    ...
    #Now compute number of years between year1 and year2 that are evenly divisible by 4, 100, 400
    ...
    #Leap years are years divisible by 4, except for years divisible by 100 that are not divisible by 400
...
def display_time (time, force_hours=False):
    ...
    time = round_to_second(time)   #round to nearest second before extracting fields
    
    if (time < 0):         #if time is negative, print a minus sign and display absolute value
        neg_string = '-'
        time = abs(time)
    else:
        neg_string = ''
    ...
    #Unless force_hours is specified, only print a field if it or a higher field is nonzero.  Fill with leading zeros.
    #The force_hours option is useful because Excel can get confused when reading short time strings.
    if (days != 0):
    ...
    if ((days == 0) and (hours_within_day == 0) and (mins_within_hour == 0)):
        sec_string = '%s' %((str(secs_within_min)))   #avoid zero fill when there are only seconds
    else:
...
def time_from_string (time_string):
    ...
    fields = (string.split(time_string, ':')) #extract fields
    seconds = int(fields[-1])
    num_fields = len(fields)
    minutes = hours = days = 0   #default to zero if not provided
...
def display_date (mjd):
    ...
    int_days = int(floor(321.0 + mjd))  #adjust to number of days since Dec. 31, 1857
    ...
    while (day_of_year < 1):   #handle case where leap year adjustment has made day negative
       year -= 1
