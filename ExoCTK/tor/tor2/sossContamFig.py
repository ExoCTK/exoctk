...
app_exoctk.py. It is part of the ExoCTK package. It creates the contamination plots that show how much and where the spctra of target is contaminated by nearby sources.
...
Usage:
	* From python script:
		from ExoCTK.tor.tor2.sossContamFig import contam
		contam(ra, dec, targetName, cubeName, pamin, pamax)
...
def contam(ra, dec, targetName, cubeName, pamin = 0, pamax = 360):
	...
	Args:
		...
		cubeName (str)   : Name of fits file. This is what 
						   sossFieldSim.py returns
		pamin (int)      : minimum position angle 0<pamin<360
		pamax (int)      : maximum position angle 0<pamax<360
	
	Returns:
		figdata_png : The plot in bytes form
	"""
...
if __name__ == "__main__":
    ...
    pamin=0 if len(argv)<6 else int(argv[5])
    pamax=360 if len(argv)<7 else int(argv[6])
