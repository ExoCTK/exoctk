...
def lmfitter(time, data, model, unc=None, method='leastsq', verbose=True):
    ...
    all_params = [i for j in [model.components[n].parameters.list for n in range(len(model.components))] for i in j]
    ...
    # Get values from input ExoCTK.lightcurve_fitting.parameters.Parameters instances
    ...
    new_params = [(fit_params.get(i).name, fit_params.get(i).value, fit_params.get(i).vary, fit_params.get(i).min, fit_params.get(i).max) for i in fit_params]
