...
class Filter(object):
    ...
    def __init__(self, band, filter_directory=None,
        ...
        if filter_directory is None:
            filter_directory = resource_filename('ExoCTK', 'data/filters/')

        # If the filter_directory is an array, load it as the filter
        if isinstance(filter_directory, (list, np.ndarray)):

            dat = filter_directory[0].data
            self.raw = np.array([dat, filter_directory[1]])
            ...
            if band.lower().replace('-', '').replace(' ', '') == 'tophat':
                ...
                if filepath not in files:
                    ...
                    if dl.lower() == 'y':
                        ...
                        band = input('Enter the band name to retrieve\
                                     (e.g. 2MASS/2MASS.J): ')

                        # Download the XML (VOTable) file
                        baseURL = 'http: //svo2.cab.inta-csic.es/svo\
                                  /theory/fps/fps.php?ID='
                        filepath = filter_directory+os.path.basename(band)
                        urllib.request.urlretrieve(baseURL+band, filepath)
    ...
    def delete(self):
        ...
        """
        filt = self.filterID
        words = 'Are you sure you want to delete {} ? [n] '.format(filt)
        do_it = input(words)
        if do_it.lower() == 'y':
            os.remove(self.path)
    ...
    def bin(self, n_bins=1, pixels_per_bin='', bin_throughput='',
        ...
        elif not n_bins and not pixels_per_bin and t_bin:
            pass
        else:
    ...
    def load_TopHat(self, wl_min, wl_max, n_pixels=100):
        ...
        """
        t_min = isinstance(wl_min, q.quantity.Quantity)
        t_max = isinstance(wl_max, q.quantity.Quantity)
        if not t_min and not t_max:
            print('Please provide an astropy.units.quantity.Quantity\
