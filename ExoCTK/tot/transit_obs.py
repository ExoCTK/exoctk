...
def calc_StartWindow(eventType, rms, ptsOrbit, numOrbits, depth, inc, aRs, period, windowSize, ecc=0, w=90., duration=None, offset=0.):
    ...
    fig, (a), (b) = core.multiplot(2, 1, ylabel='', xlabel='', fontsize=20, figsize=(15, 7), sharex=True, sharey=True)
    a.errorbar(obsphase1, obstr1, rms, fmt='go')
    a.plot(phase1, trmodel1, 'b-', lw=2)
    ylim = a.get_ylim()
    xlim = a.get_xlim()
    b.errorbar(obsphase2, obstr2, rms, fmt='ro')
    b.plot(phase2, trmodel2, 'b-', lw=2)
    a.set_xlim(xlim)
    a.set_ylim(ylim)
